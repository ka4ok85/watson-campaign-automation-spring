...
public class ImportTableCommandTest {

	@Autowired
	ApplicationContext context;

	private String defaultRequest = String.join(System.getProperty("line.separator"),
			"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>", "<Envelope>", "<Body>", "<ImportTable>",
			"<MAP_FILE>testmap.xml</MAP_FILE>", "<SOURCE_FILE>testsource.csv</SOURCE_FILE>",
			"<FILE_ENCODING>iso-8859-1</FILE_ENCODING>", "</ImportTable>", "</Body>", "</Envelope>");

	@Test
	public void testBuildXmlDefaultRequest() {
		// get XML from command
		ImportTableCommand command = new ImportTableCommand();
		SFTP sftp = mock(SFTP.class);
		command.setSftp(sftp);
		ImportTableOptions options = mock(ImportTableOptions.class);
	...
	public void testReadResponse() {
		ImportTableCommand command = context.getBean(ImportTableCommand.class);
		ImportTableOptions options = mock(ImportTableOptions.class);
		...
		ResponseContainer<ImportTableResponse> responseContainer = command.readResponse(jobPollingContainer,
				jobResponse, options);
		ImportTableResponse response = responseContainer.getResposne();
